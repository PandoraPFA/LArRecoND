/**
 *  @file   CheatingConfigurableVertexCreationAlgorithm.cc
 *
...
CheatingConfigurableVertexCreationAlgorithm::CheatingConfigurableVertexCreationAlgorithm() :
...
StatusCode CheatingConfigurableVertexCreationAlgorithm::Run()
{
    ...
    if(m_useNeutrinoInteractionVertex)
    {
    ...
    else
    {
        ...
        const MCParticle *const pMCParticle = mcPrimaryVector.front();

        parameters.m_position = CartesianVector(
            pMCParticle->GetVertex().GetX() + m_vertexXCorrection, pMCParticle->GetVertex().GetY(), pMCParticle->GetVertex().GetZ());
        parameters.m_vertexLabel = VERTEX_INTERACTION;
        parameters.m_vertexType = VERTEX_3D;
    }
...
StatusCode CheatingConfigurableVertexCreationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
        STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle, "UseNeutrinoInteractionVertex", m_useNeutrinoInteractionVertex));
